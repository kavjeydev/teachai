...
export default function ProfilePage() {
  ...
  const creditHistory = useQuery(api.subscriptions.getCreditHistory, { limit: 50 }); // Increased limit for better search results

  const currentTier = subscription?.tier || 'free';
  const creditsUsagePercent = credits ?
    parseFloat(((credits.usedCredits / credits.totalCredits) * 100).toFixed(2)) : 0;
  ...
  const filteredCreditHistory = useMemo(() => {
    ...
    return creditHistory.filter((transaction) => {
      ...
      return (
        ...
        new Date(transaction.timestamp).toLocaleDateString().includes(searchLower) ||
        new Date(transaction.timestamp).toLocaleTimeString().includes(searchLower)
      );
    });
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <CardTitle className="flex items-center gap-3">
                      ...
                      <Badge className={`${getTierBadgeColor(currentTier)} text-white`}>
              ...
              <CardHeader className="flex-shrink-0">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      Your AI model usage history ({filteredCreditHistory.length} {filteredCreditHistory.length === 1 ? 'transaction' : 'transactions'})
                    </CardDescription>
                  </div>
                </div>
