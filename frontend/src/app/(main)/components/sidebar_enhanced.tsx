...
export function AppSidebar({
  ...
  const [sortBy, setSortBy] = React.useState<"date" | "name" | "activity">("date");
  ...
  const filteredAndSortedChats = React.useMemo(() => {
    ...
    let filtered = chats.filter((chat) => {
      ...
      const matchesFolder = selectedFolder === "all" || (chat as any).folder === selectedFolder;
  ...
  const folders = React.useMemo(() => {
    if (!chats) return [];
    const folderSet = new Set(chats.map((chat: any) => chat.folder || "general").filter(Boolean));
            ...
            {showFilters && (
              <div className="bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-900 rounded-xl p-4 mb-4 space-y-4 border border-slate-200 dark:border-slate-700">
                <div>
                  ...
                  <select
                    value={sortBy}
                    onChange={(e) => setSortBy(e.target.value as "date" | "name" | "activity")}
                ...
                <div className="pt-2 border-t border-slate-200 dark:border-slate-700">
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="text-center p-2 bg-white dark:bg-slate-800 rounded-lg">
                      <div className="font-bold text-trainlymainlight">{chats?.length || 0}</div>
                      <div className="text-slate-500">Total</div>
                    </div>
                    <div className="text-center p-2 bg-white dark:bg-slate-800 rounded-lg">
                      <div className="font-bold text-trainlymainlight">{filteredAndSortedChats.length}</div>
              ...
              {filteredAndSortedChats.length === 0 ? (
                <div className="text-center py-8">
                  <div className="w-12 h-12 bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-900 rounded-xl flex items-center justify-center mx-auto mb-3">
                    {searchQuery ? <Search className="w-6 h-6 text-slate-400" /> : <MessageSquare className="w-6 h-6 text-slate-400" />}
                  </div>
                          ...
                          >
                            ...
                            <div className="flex items-center gap-3 text-xs text-slate-500 dark:text-slate-400">
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <span>{new Date(chat._creationTime).toLocaleDateString()}</span>
                              </div>
                            </div>
                          </button>
                        )}
