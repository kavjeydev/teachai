...
export function useFileUploadState() {
  ...
  const uploadFile = useCallback(
    async (
      ...
      return api.execute(() => uploadFn(file, api.setProgress), {
...
export function useGraphApiState() {
  ...
  const loadGraphData = useCallback(
    async (chatId: string, baseUrl: string, options: ApiOptions = {}) => {
      return api.execute(
        async () => {
          api.setProgress(20);
  ...
  const answerQuestion = useCallback(
    async (payload: any, baseUrl: string, options: ApiOptions = {}) => {
      return api.execute(
        async () => {
          api.setProgress(10);
