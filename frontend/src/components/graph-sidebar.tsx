...
export const GraphSidebar: React.FC<GraphSidebarProps> = ({
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "fixed right-0 top-0 h-full bg-white/98 dark:bg-slate-900/98 backdrop-blur-xl border-l border-slate-200 dark:border-slate-700 shadow-2xl z-50 transition-all duration-300 ease-out",
          isOpen
            ? isCollapsed
              ? "w-16 translate-x-0"
        ...
        <div className="flex-1 h-full overflow-hidden">
          {!isCollapsed ? (
            ...
            <div className="flex flex-col items-center py-8 space-y-4">
              <div className="w-10 h-10 bg-gradient-to-br from-trainlymainlight to-purple-600 rounded-xl flex items-center justify-center shadow-lg animate-pulse">
                <Network className="w-5 h-5 text-white" />
              </div>
              <div className="text-xs text-slate-500 dark:text-slate-400 writing-mode-vertical transform rotate-90 mt-4">
                Graph
              </div>
            </div>
          )}
