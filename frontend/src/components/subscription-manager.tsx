        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div
                className={`w-10 h-10 bg-gradient-to-br ${getTierColor(currentTier)} rounded-xl flex items-center justify-center text-white`}
              >
          ...
          {creditsUsagePercent > 80 && (
            <div className="flex items-center gap-2 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
              ...
              <p className="text-sm text-amber-800 dark:text-amber-200">
                You're running low on credits. Consider upgrading or purchasing
                additional credits.
              </p>
            </div>
          )}
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {Object.entries(PRICING_TIERS)
                ...
                .map(([key, tier]) => (
                  <div
                    key={key}
                    className="border border-zinc-200 dark:border-zinc-700 rounded-lg p-4 hover:border-amber-400/50 transition-colors"
                  >
                    <div className="flex items-center gap-2 mb-3">
                      <div
                        className={`w-8 h-8 bg-gradient-to-br ${getTierColor(tier.id)} rounded-lg flex items-center justify-center text-white`}
                      >
                        {getTierIcon(tier.id)}
                      </div>
                      <div>
